...
import NarrowSearchBar from "../SearchBar/NarrowSearchBar/NarrowSearchBar";
import ExpandedSearchBar from "../SearchBar/ExpandedSearchBar/ExpandedSearchBar";
import UserProfile from "../UserProfile/UserProfileComponent";
import NavContents from "../NavContents/NavContents";

const Header = () => {
  const [centerContent, setCenterContent] = useState("expandedSearch");
  const [headerStyle, setHeaderStyle] = useState("dark");
  const [logoColor, setLogoColor] = useState("white");
  const [selectedNavContent, setSelectedNavContent] = useState("places");
  ...
  const listenScrollEvent = () => {
    if (window.scrollY < 5) {
      setCenterContent("expandedSearch");
      setHeaderStyle("dark");
      setLogoColor("white");
    } else if (window.scrollY > 5) {
      setCenterContent("narrowSearch");
      setHeaderStyle("light");
      setLogoColor("#ff385c");
    }
  };

  const selectedContentCallback = (content) => {
    setSelectedNavContent(content);
  }
  return (
    <div className={`${headerStyle}`}>
      <div className="header-wrapper justify-content-space-between">
        <div>
          <svg
        ...
        {centerContent === "expandedSearch" ? (
          <NavContents selectedContentItem={selectedContentCallback} />
        ) : (
          <NarrowSearchBar />
        )}
      ...
      <div className="header-wrapper justify-content-center">
        {centerContent === "expandedSearch" ? <ExpandedSearchBar selectedNavContent={selectedNavContent} /> : ""}
      </div>
    </div>
  );
};
